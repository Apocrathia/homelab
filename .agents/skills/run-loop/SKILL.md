...
### Ship behavior by mode

| Mode         | Ship behavior                                                                                                                                                                                                                                                                                                                                         |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scout`      | Never ships — no commit/push/MR open as ship. Read-only find + optional file-issue + notify only.                                                                                                                                                                                                                                                     |
| `unattended` | Never commit/push to `main`. When authorized for this lap (soft ship or explicit `commit`/`push`; [`constraints.md#commit-and-ship`](../../context/constraints.md#commit-and-ship)), follow [`draft-commit`](../draft-commit/SKILL.md) **autonomous** path: feature branch + MR (draft by default). Otherwise draft-commit handoff only. Never merge. |
| `attended`   | When authorized ([`constraints.md#commit-and-ship`](../../context/constraints.md#commit-and-ship)): may ship to `main` per [`draft-commit`](../draft-commit/SKILL.md) attended path. Otherwise draft-commit handoff only.                                                                                                                             |
...
| G8   | Ship without authorization, or to `main` when mode forbids it       | `scout` — never ship; `unattended` — never commit/push/merge to `main` (authorized autonomous ship via [`draft-commit`](../draft-commit/SKILL.md) only); `attended` — may ship to `main` when authorized ([`constraints.md#commit-and-ship`](../../context/constraints.md#commit-and-ship)); else draft-commit handoff only |
...
- Ship per mode ([`constraints.md#commit-and-ship`](../../context/constraints.md#commit-and-ship),
  [`draft-commit`](../draft-commit/SKILL.md)): `scout` never ships;
  `unattended` never commit/push/merge to `main` (authorized → autonomous
  feature branch + MR; else handoff); `attended` may ship to `main` when
  authorized, else handoff. Never merge an MR.
...
| [`draft-commit`](../draft-commit/SKILL.md)                    | Commit/MR handoff; ships per authorization (attended → `main`; autonomous → feature branch + MR) |
...
- Advice ≠ implement (G3).
- Operator owns merge and any live apply / Flux reconcile; commit/push per
  authorization via [`draft-commit`](../draft-commit/SKILL.md) (see ship
  behavior by mode above).
