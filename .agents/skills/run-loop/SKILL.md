...
| Mode         | Behavior                                                                                                                                                                                     |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scout`      | `find-work` → optional [`file-issue`](../file-issue/SKILL.md) for new gaps → Discord or lap-report summary → **STOP** (no [`implement-change`](../implement-change/SKILL.md)). Cron default. |
| `unattended` | Walk briefs 1→N (max **3** tries default); only AFK-eligible; first success wins; **STOP**. One implement lap max per session.                                                               |
| `attended`   | Operator-driven; may run [`alignment`](../alignment/SKILL.md); protected paths need confirm.                                                                                                 |
...
- [ ] 3. Apply stop gates G1–G16 before any mutate / implement
- [ ] 4. Mode fork:
         scout      → optional file-issue for new gaps → notify + lap-report → STOP
         unattended → walk 1→N AFK-eligible only; first success → one implement lap max → STOP
         attended   → operator may pick / waive; alignment OK; confirm protected
- [ ] 5. On implement win: implement-change → review-loop → reconcile-docs →
         reconcile-context → draft-commit (never commit)
...
| G15  | `scout` mode after find (+ optional file-issue)                  | Notify + lap-report; **no implement-change**                   |
| G16  | Lap budget: unattended already completed one implement lap       | Stop; do not start a second implement                          |
...
- `scout` never calls `implement-change` (G15).
...
| [`watch-mr`](../watch-mr/SKILL.md)                         | MR maintain + hot locks (Wave 5)           |
...
- No Discord-as-backlog; no inventing `autoresearch` busywork here (Wave 6).
