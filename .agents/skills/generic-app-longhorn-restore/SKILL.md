...
- **Out of scope**:
  - **CNPG** instance PVCs (`*-postgres-1`): full logical flow (extract → new cluster → restore) lives in **`.agents/skills/cnpg-logical-database-restore/SKILL.md`**. This file still describes **PV/PVC / Longhorn** mechanics you reuse for that PVC; it does not cover **`pg_dump` / `pg_restore`**.
