...
# sign-in button). allow_syncable_passkeys stays at the org default: it is
# Early Access gated (Passkeys Rebrand) and apply fails where the EA feature is
# off. Add more entries to authenticators to manage email/phone/etc.
#
...
inputs = {
  ...
  webauthn = {
    # allow_syncable_passkeys omitted: EA-gated (Passkeys Rebrand); org default applies.
    attachment                         = "ANY"
