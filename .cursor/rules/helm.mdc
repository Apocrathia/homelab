---
description: Helm charts, templates, and values under helm/
globs: helm/*
alwaysApply: false
---
...
- **Always validate proposed Helm values against the chart defaults** when suggesting Helm configurations.
...
- **Use the dashboard-icons CDN (homarr-labs)** for chart and blueprint icons: `https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/{svg|png}/{service-name}.{ext}`.
- For deployment-local icons (icons committed to the repo), use the GitLab raw URL pattern documented in `.agents/skills/helm-deployment/SKILL.md`.
