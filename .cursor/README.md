...
- **Agents, skills, commands, memories, plans** are not auto-loaded. Agents discover them by reading the relevant `README.md` and pulling the specific file. Note: Cursor's IDE planning UI surfaces files under `plans/` natively.

## Adding new content

- New rule: drop a `.mdc` file in `rules/` with appropriate frontmatter, then add a row to `rules/README.md`.
- New persona, skill, command, memory, or plan: create the file, add an entry to that subdir's README.
