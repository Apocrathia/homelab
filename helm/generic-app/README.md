...
loadbalancer:
  ...
  labels: # Optional labels for Cilium IP pool selection
...
**Shared IP with Labels:**

Multiple services can share the same LoadBalancer IP by using matching labels with a `CiliumLoadBalancerIPPool`:

```yaml
# In CiliumLoadBalancerIPPool
spec:
  serviceSelector:
    matchLabels:
      apocrathia.com/ingest: "true"

# In each service's HelmRelease
loadbalancer:
  enabled: true
  ip: "10.100.1.96"
  labels:
