...
- **Module (Remote)**: the Workspace is a thin shell — `source: Remote` pulls `terraform/modules/authentik-app` pinned to the `generic-app-0.0.84` chart release tag (the tag already exists — the module landed on main with it), `kustomization.yaml` renders only the three tofu docs, and the module inputs live in the Workspace `varmap` (the full input set — no chart composer here; variable contract: the module's `variables.tf`). Three live-vs-default overrides ride the varmap, verified in the authentik DB: `internal_host` points at the dashboard service `http://chaos-dashboard.chaos-mesh.svc.cluster.local:2333`, not the app-name pattern the chart would compose; `intercept_header_auth: true` (module default false); and `access_token_validity: "minutes=10"` (the TF default written at blip time; the module proxy default is `hours=1`). NOT shared: live carries only the admins binding (no users binding, no tailnet parentRef).
- **Same-state gate**: the module switch keeps this app's existing tofu state — the gate renames the inline-era tofu state addresses to the module's addresses via `tofu state mv` (workspace paused + guarded, Flux suspended), so the first plan reads `No changes` — no wipe, no import ids anywhere in git, uuids intact, nobody re-logs in. The one-time gate sequence (chaos-mesh's 4 address moves) rides the MR description; the paused workspace must be UNPAUSED as part of the resume step — a paused workspace never processes its own deletion (the demo-app gate proved this live).
