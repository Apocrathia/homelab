...
3. Register the agent in LiteLLM `agents:` (`agent_card_params.url`) with A2A URL `/api/a2a/<namespace>/<agent-name>`. Clients call LiteLLM native `POST /a2a/{agent_name}` (discover via `GET /v1/agents`).
