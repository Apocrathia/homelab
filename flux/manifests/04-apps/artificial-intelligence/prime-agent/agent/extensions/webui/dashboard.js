...
function mdRender(text, depth) { // -> DocumentFragment of block elements
  ...
  if (typeof marked !== 'undefined' && typeof DOMPurify !== 'undefined' && DOMPurify.isSupported) {
    ...
    for (const c of t.content.querySelectorAll('code')) codeDialect(c); // 7ab-b: fenced ```python blocks tokenize in place (inline `chips` carry no language class — unchanged)
    return t.content;
  }
...
function mdRenderHouse(text, depth) { // in-house fallback — copy the TUI, no deps
  ...
  while (i < lines.length) {
    ...
    if (MD_FENCE_OPEN_RE.test(ln)) { flushAll();
      ...
      if (lang.toLowerCase() === 'python') code.appendChild(hlPython(body.join('\n'))); // 7ab-b: the in-house python tokenizer (the lib-path walk's parity for the fallback renderer); 'python' exact — ```py stays plain (the frozen md battery pins it)
      else code.textContent = body.join('\n');
...
// 7ab-b/cell-kind: in-house CODE tokenizers — the jsonHighlight regex family
// extended (no vendored dep, ~30 lines). Two dialects share one scanner:
// python (comments/strings/def-names/keywords/numbers) and bash (strings/
// $vars/comments/keywords/operators/numbers). Fenced ```python blocks in
// rendered markdown + the ipython toolcard's code cells render through it;
// every token lands via createTextNode/textContent — escaped by
// construction, like the rest of the renderer family.
const PY_RE = /(#[^\n]*)|([rRbBuUfF]{0,3}(?:'''[\s\S]*?'''|"""[\s\S]*?"""|'(?:\\.|[^'\\\n])*'|"(?:\\.|[^"\\\n])*"))|(\b(?:def|class)\s+\w+)|(\b(?:and|as|assert|async|await|break|case|continue|def|del|elif|else|except|False|finally|for|from|global|if|import|in|is|lambda|match|None|nonlocal|not|or|pass|raise|return|True|try|while|with|yield)\b)|(\b\d+(?:\.\d+)?(?:[eE][+-]?\d+)?\b)/g;
const SH_RE = /(\$\{[^}]*\}|\$[#?$!@*]|\$\w+)|(#[^\n]*)|('(?:\\.|[^'])*'|"(?:\\.|[^"\\])*")|(\b(?:if|then|elif|else|fi|for|while|until|do|done|case|esac|function|select|in|time|return|exit|export|local|source|set|unset|echo|cd)\b)|(2>&\d|\d?>{1,2}|\d?<&?\d+|\|\||&&|[|;&])|(\b\d+(?:\.\d+)?\b)/g;
function hlCode(code, RE, cls) { // one regex, one span-class picker -> a fragment (the jsonHighlight shape)
...
function hlPython(code) { return hlCode(code, PY_RE, (m) => m[1] ? 'c-com' : m[2] ? 'c-str' : m[3] ? 'c-def' : m[4] ? 'c-kw' : 'c-num'); }
function hlBash(code) { return hlCode(code, SH_RE, (m) => m[1] ? 'c-var' : m[2] ? 'c-com' : m[3] ? 'c-str' : m[4] ? 'c-kw' : m[5] ? 'c-op' : 'c-num'); }
function codeDialect(c) { // 7ab-b: post-sanitize code walk — python fences tokenize in place (marked's language-* classes)
  const lang = ((((c.className || '') + '').match(/language-([\w+-]+)/)) || [])[1] || '';
  if (lang.toLowerCase() !== 'python') return; // 'python' exact — other tags (incl. py) stay plain
...
function ipyCell(args) { // {code, rest, kind, command} when the args parse and carry a code string; null otherwise. kind: python | bash
  ...
  const command = ipyBashKind(a.code);
  return command !== null ? { code: a.code, rest, kind: 'bash', command } : { code: a.code, rest, kind: 'python' };
}
// cell-kind detection — the payload verdict (617/617 this session, 3,429/3,429
// fleet-wide): ipython toolCall args ALWAYS carry a python-source .code; NO
// command field, NO sub-kind marker — shell commands ride INSIDE the code as
// `h = bash('cmd')` wrappers (59 here). Detection is therefore code-text
// shaped: the bash()-wrapper pattern (first statement an optional-assignment
// [await] bash('...') with only trivial tail statements — output prints,
// handle awaits, sleeps); the command string extracts and unescapes. A
// raw-shell fallback (shebang / lead-word + shell operators — 0 in the wild,
// cheap insurance) covers hand-shaped payloads. Everything else = python.
const IPY_BASH_HEAD = /^[ \t]*(?:[A-Za-z_]\w*\s*=\s*)?(?:await\s+)?bash\s*\(\s*/;
function ipyStrLit(s, i) { // -> index just past the closing quote of the literal at s[i], or -1 (escapes honored)
  const t = s.slice(i, i + 3), triple = (t === "'''" || t === '"""');
  if (triple) { const e = s.indexOf(t, i + 3); return e < 0 ? -1 : e + 3; } // 7at-triple: the triple branch runs BEFORE the single-quote guard — the multi-char OPEN delimiter is neither a single nor a double quote, the guard rejected it and left this branch DEAD (the batch-A-review residual: 7/40 real cells mislabeled python)
  const open = s[i];
  if (open !== "'" && open !== '"') return -1;
  for (let k = i + 1; k < s.length; k++) {
    if (s[k] === '\\') { k++; continue; } // an escaped char (incl. quotes) — skip its char
    if (s[k] === open) return k + 1;
    if (s[k] === '\n') return -1; // single-quoted literals never span lines
  }
  return -1;
}
function ipyRawShell(code) { // fallback: a bare shell command as the code (shebang or a lead word + shell operators) — never seen live, cheap insurance
  if (!/^[ \t]*(?:#!|(?:(?:sudo\s+)?(?:cd|kill|lsof|ps|grep|cat|ls|echo|export|mkdir|rm|cp|mv|sed|awk|find|head|tail|sort|uniq|wc|chmod|chown|touch|which|env)\b))/.test(code)) return null;
  if (!/(?:&&|\|\||\||>>|>|2>&1|\$\w|\$\{)/.test(code)) return null;
  return code;
}
function ipyBashKind(code) { // the extracted command when the cell is a bash() wrapper; null = a python cell
  const m = IPY_BASH_HEAD.exec(code);
  if (!m) return ipyRawShell(code);
  const pm = /^[rRbBuUfF]{0,3}/.exec(code.slice(m[0].length)); // a literal prefix (f/r/b...) may lead the command string
  const q = ipyStrLit(code, m[0].length + pm[0].length);
  if (q < 0 || code.indexOf(')', q) < 0) return null; // unterminated literal/call — treat as python, never crash the card
  for (const ln of code.slice(code.indexOf(')', q) + 1).split('\n')) { // the tail must be trivial plumbing, never substantive python
    const t = ln.trim();
    if (!t || t.startsWith('#')) continue;
    if (!/^(?:await\s+(?:asyncio\.)?sleep\([^)]*\)|[A-Za-z_]\w*\s*=\s*await\s+[A-Za-z_]\w*(?:\.output\(\))?|await\s+[A-Za-z_]\w*(?:\.output\(\))?|print\()/.test(t)) return null;
  }
  const lit = code.slice(m[0].length, q);
  const inner = (lit.startsWith("'''") || lit.startsWith('"""')) ? lit.slice(3, -3) : lit.slice(1, -1);
  return inner.replace(/\\(['"\\nrt])/g, (x, c) => (c === 'n' ? '\n' : c === 'r' ? '\r' : c === 't' ? '\t' : c)); // the REAL command: escapes resolve
}
function toolCard(id, name, status, args) {
  ...
  txt(t.nm, (t.ipy ? t.ipy.kind : name) + (status==='running' ? ' — running' : status==='error' ? ' — error' : '')); // cell-kind: an ipython card labels its DETECTED kind (python | bash) — "some ipython cells are actually just bash"; other tools keep their name
  if (args !== undefined) tcArgs(t, args);
  return t;
}
function tcArgs(t, args) { // cell-kind: code cells render tokenized (python cells in full; bash cells show the extracted COMMAND, bash-highlighted — the `h = bash(...)` wrapper is plumbing, not content); raw args for non-code tools
  if (t.ipy) {
    t.ar.className = 'tcode';
    t.ar.textContent = '';
    t.ar.appendChild(t.ipy.kind === 'bash' ? hlBash(t.ipy.command || t.ipy.code) : hlPython(t.ipy.code));
    if (t.ipy.rest) t.ar.appendChild(document.createTextNode('\n· ' + t.ipy.rest));
...
// 7ab-b: EMBEDDED JSON — the operator re-report: agent messages embed JSON
// INSIDE prose (verdict lines + the blob), so the whole-text parse gate above
// misses it. Detection: fenced ```json blocks + brace-balanced spans within
// the prose; each candidate parses, pretty-prints (2-space) + highlights IN
// PLACE (.j-* spans); non-JSON prose stays plain; unparseable/overlapping
// candidates fall back to raw text. All output via text nodes — escaped by
// construction, like the rest of the family.
function spanClose(t, i) { // index of the bracket closing t[i] (string-aware), or -1 — the brace-balance scan
...
function jsonProseSpans(text) { // one prose segment: brace-balanced JSON spans highlighted in place, the rest raw text
  const frag = document.createDocumentFragment();
  let last = 0, i = 0;
  while (i < text.length) {
    const c = text[i];
    if ((c === '{' || c === '[')) {
      const j = spanClose(text, i);
      if (j > i) {
        const sp = text.slice(i, j + 1);
        let v; try { v = JSON.parse(sp); } catch (e) { v = null; }
        if (v !== null && typeof v === 'object') { // scalars ("[42]") never transform — content must survive verbatim
          if (i > last) frag.appendChild(document.createTextNode(text.slice(last, i)));
          frag.appendChild(jsonHighlight(sp));
          last = i = j + 1; continue;
        }
      }
    }
...
function jsonEmbedRender(text) { // the agentmsg body: whole-JSON keeps the 7ab treatment; embedded blocks/spans render in place
  const t = String(text ?? '');
  const whole = jsonHighlight(t);
  if (whole) return whole;
  const frag = document.createDocumentFragment();
  for (const part of t.split(/(```json[ \t]*\r?\n[\s\S]*?```)/gi)) { // fenced ```json blocks lift out whole
    const m = /^```json[ \t]*\r?\n([\s\S]*?)```$/i.exec(part);
    const hi = m ? jsonHighlight(m[1]) : null; // fence content must parse — a broken fence renders as raw prose
    if (hi) frag.appendChild(hi);
    else frag.appendChild(jsonProseSpans(part));
  }
  return frag;
}
  ...
  switch (item.kind) {
    ...
    case 'assistant':
      ...
      if (streaming) { liveNode = r; } else {
        ...
        // 7ap-2.0 typewriter dump gate (operator re-report: "i'm still seeing
        // text instantly appear"): the emulation fires for live-path FINAL
        // items whose text arrived as a DUMP SHAPE — <=1 text-bearing live
        // event (0 = the silent class: a wait + one burst, no partial text
        // ever existed upstream; 1 = the provider-buffered class: one
        // full-text delta, ~19% of glm-5.3-prime completions). Both reveal
        // paced; a genuinely streamed message (2+ deltas) never emulates.
        if (live && !streamed && item.text) typewriterReveal(r.body, item.text);
      }
      break;
    case 'tool':
    ...
    case 'custom': endLive();
      ...
      // them only in Expanded (no toolcard class).
        ...
        bd.appendChild(jsonEmbedRender(item.text || '')); // 7ab-b: EMBEDDED JSON — fenced ```json blocks + brace-balanced spans pretty + highlighted IN PLACE; prose untouched (whole-JSON bodies keep the 7ab treatment)
...
// 7ap-2.0 fast-forward: the typewriter emulation's skip button — lazy like
// the pill, the bottom-LEFT mirror (grey-4 glyph + hairline, subtle). Shows
// ONLY during an active emulation — and only when it's long enough to be
// skippable (>= TW_FF_MS; a sub-second reveal flashing a button is noise,
// not signal). Click = jump-to-full + hide; completion auto-hides.
let ffBtn = null;
function ffHide() { if (ffBtn) ffBtn.style.display = 'none'; }
function ffShow() {
  if (!feed.parentNode) return; // fake-DOM batteries have no #session anchor — the reveal itself is the pin there
  if (!ffBtn) {
    ffBtn = document.createElement('button');
    ffBtn.className = 'feedff';
    txt(ffBtn, '» skip'); // "this one just flew in — here's the button if you're in a rush" (operator design)
    ffBtn.addEventListener('click', () => twFinish()); // jump to the full text + hide (twFinish hides with the reveal)
    feed.parentNode.appendChild(ffBtn); // inside #session like the pill — snapshot wipes must not destroy it
  }
  const comp = $('#composer'), qb = $('#queue');
  ffBtn.style.bottom = (((comp && comp.offsetHeight) || 0) + ((qb && qb.offsetHeight) || 0) + 12) + 'px';
  ffBtn.style.display = 'inline-block';
}
// 7ap typewriter 2.0 — ponytail: PURE COSMETIC client-side smoothing for
// gateway-buffered arrivals (the upstream never streamed partial text, so
// nothing real is being faked — the dump just gets paced). 2.0 (operator
// design): the dump gate extends to the SINGLE-DELTA class (the message's
// text arrived in <=1 text-bearing live event — the liveTextSeen counter
// decides; silent AND single-delta finals both reveal, a real stream never
// emulates), and the reveal PACES at the OBSERVED rate — the burst strip's
// rolling tok/s median (duration = tokens/rate, capped so huge dumps don't
// crawl) — while the fast-forward button floats bottom-left (skip if you're
// in a rush). window.__TW_FAST (tests) completes on the first tick. Upgrade
// path if this ever reads as fake: drop it — the fix for the bimodal gateway
// is upstream (LiteLLM router config), not here.
let tw = null; // the running reveal: { iv, body, text, time }
const TW_STEP_MS = 30, TW_MIN_MS = 300, TW_CAP_MS = 4500, TW_FALLBACK_MS = 720, TW_FF_MS = 1000; // 30ms frames; duration floor/cap; no rate samples -> the flat legacy ~720ms; the FF button shows only for reveals >= 1s (shorter = a flash of noise)
function twRate() { // the rolling median — the observed speed the reveal emulates
  if (!burstRates.length) return null;
  const a = [...burstRates].sort((x, y) => x - y);
  return a[Math.floor(a.length / 2)];
}
function twDuration(text) { // tokens/rate, capped; no observed rate -> the flat legacy pace
  const rate = twRate();
  if (!rate) return TW_FALLBACK_MS;
  return Math.min(TW_CAP_MS, Math.max(TW_MIN_MS, Math.ceil(text.length / 4 / rate * 1000)));
}
function twFinish() { // render the full text instantly (endLive mid-reveal, the FF click, or the last tick)
  if (!tw) return;
  clearInterval(tw.iv);
  const w = tw; tw = null;
  ffHide(); // 7ap-2.0: the skip button dies with the reveal (auto-hide at completion)
...
function typewriterReveal(body, text) {
  ...
  const dur = twDuration(text);
  const frames = Math.max(2, Math.round(dur / TW_STEP_MS)); // >=2: a prefix frame + the finish
  let n = 0;
  const step = Math.max(1, Math.ceil(text.length / frames)); // short messages finish in their own length, not stretched ticks
  ...
  if (dur >= TW_FF_MS) ffShow(); // 7ap-2.0: the skip button — only while an emulation long enough to be skippable runs
  ...
  tw.iv = setInterval(() => {
    ...
    if ((typeof window !== 'undefined' && window.__TW_FAST) || n >= frames || k >= text.length) { twFinish(); return; }
    paint(k);
  }, TW_STEP_MS);
}
...
// 7ap-2.0 rate window: a finished inference burst folds its tok/s into the
// rolling sample set the typewriter paces at (the observed-rate median).
// Dump bursts never qualify — a single full-text delta has a zero-elapsed
// window; tiny/sub-half-second windows are noise. Once per burst (done flag).
const burstRates = []; // declared HERE (next to its writer) — the busy-region slices carry it; the fn-region batteries stub it
function commitBurst() {
  const b = turn && turn.burst;
  if (!b || b.done) return;
  b.done = true;
  const sec = (b.lastAt - b.startAt) / 1000, down = Math.ceil((turn.textChars + turn.thinkingChars) / 4);
  if (sec >= 0.5 && down - b.baseDown >= 8) burstRates.push((down - b.baseDown) / sec);
  if (burstRates.length > 8) burstRates.shift(); // rolling: the last 8 observed bursts
}
function busyReset(promptChars, startedAt) { commitBurst(); turn = { start: (startedAt && startedAt > 0) ? startedAt : null, promptChars: promptChars || 0, textChars: 0, thinkingChars: 0, label: 'Waiting', lastDeltaAt: 0, toolAt: 0, burst: null }; } // honest timer: no anchor -> --:--; 7ap-2.0: the closing burst feeds the rate window first
function busyUpdate(d) {
  ...
  if (d.turnTextChars != null || d.turnThinkingChars != null) { // a live token delta
    ...
    if (!turn.burst || turn.toolAt > turn.burst.lastAt || now - turn.lastDeltaAt > BURST_GAP) {
      commitBurst(); // 7ap-2.0: the window that just closed feeds the rate median
  ...
  if (d.tool) { commitBurst(); turn.toolAt = now; } // inference paused — the burst freezes at its last delta (and feeds the rate window)
...
function setWorking(on) {
  ...
  if (on) {
    ...
    busyStart = null; commitBurst(); turn = null; // 7ap-2.0: the turn's last burst feeds the rate window before the counters die
...
searchEl.addEventListener('keydown', (e) => { if (e.key === 'Escape') { searchEl.value = ''; searchQ = ''; renderList(); searchXVis(); } }); // clear-on-Escape (the X hides with it)
// 7an clear-X: the search input's own clear affordance — a hairline × on the
// input's right, VISIBLE ONLY while text is present; a click clears the query,
// re-renders the full list, and keeps focus in the input.
const searchX = document.createElement('button');
searchX.className = 'search-clear'; searchX.type = 'button'; searchX.title = 'clear search';
txt(searchX, '\u00d7');
searchEl.parentNode.insertBefore(searchX, searchEl.nextSibling); // right of the input, before the count
const searchXVis = () => { searchX.style.display = searchEl.value ? 'inline-block' : 'none'; };
searchX.onclick = () => { searchEl.value = ''; searchQ = ''; renderList(); searchXVis(); searchEl.focus(); };
searchEl.addEventListener('input', searchXVis);
searchXVis(); // the honest first paint: a pre-filled input shows the X immediately
...
function renderRow(s, box, depth, keep) { // keep: 7an search predicate (undefined = unfiltered, all children render)
  ...
  if (s.archived) row.classList.add('arch'); // G4 badge: the disk state says archived (the LAST session_state)
  ...
  if (s.stale) { const t = document.createElement('span'); t.className = 'rtag'; txt(t, 'stale'); main.appendChild(t); } // row-state: the micro tag (badge.idle's outline dialect, no accent)
  if (s.archived) { const t = document.createElement('span'); t.className = 'rtag'; txt(t, 'arch'); main.appendChild(t); } // G4: the archived badge
...
async function showSession(id, restore, force) { // selection model: one click loads a session into the pane — no views, no router; force: the 7aq/48 self-heal reloads must pass the re-select guard
  ...
  draftSave(); // 7as: the leaving chat's composer saves into ITS slot — sid still points at the old chat here, the first line past the guard
  const tok = ++selToken;
...
// draft is the UNSENT text ONLY: a send clears the slot the moment the text
// leaves the composer (idle echo or busy queue — the queue strip is server
// truth and never mirrors into drafts), a send failure re-saves the restored
// text (growInput's autosave), and the showSession chokepoint above saves the
// leaving chat's composer before the entering chat's draft loads. Autosave
// rides growInput: every input event, picker accept, and failure rollback
// passes through it.
const DRAFT_KEY = 'webui-draft-';
function draftSave() { if (sid) lsSet(DRAFT_KEY + sid, input.value); } // no open pane -> no slot written
function draftLoad() { input.value = sid ? (lsGet(DRAFT_KEY + sid) || '') : ''; growInput(); } // growInput's autosave writes the loaded text straight back — same value, no churn
function draftClear() { if (sid) lsSet(DRAFT_KEY + sid, ''); } // send(): the text is OUT (echo or queued) — the slot empties
function attachEvents(id) {
  ...
  es.addEventListener('snapshot', (e) => {
    ...
    if (!live) { if (sid === id) showSession(id, undefined, true); return; } // 7aq: this stream was the not_live-retrying one a resume armed — the snapshot means the beacon registered: reload the pane live (a stale stream never steals the selection; force bypasses the 7aw re-select guard)
    feed.innerHTML = ''; for (const k in toolStatus) delete toolStatus[k]; endLive();
...
// Feed items: Copy text (the RAW source). Clipboard writes toast; headless
// boards degrade to a notice.
let cm = null;
...
async function cmDelete(id) { // row-state: the reaper's client half — POST /api/delete-agent flips the child's rlm-subagent ledger to 'deleted' (the row re-renders inactive; the transcript stays on disk)
  let st = 0, jb = null;
  try {
    const r = await fetch('/api/delete-agent', {
      method: 'POST', headers: { 'content-type': 'application/json', 'x-prime-token': token },
      body: JSON.stringify({ id }) });
    st = r.status; const text = await r.text();
    try { jb = JSON.parse(text); } catch (e) {}
  } catch (e) {}
  if (!cm) return; // closed mid-flight
  closeCm();
  if (st === 409) toast('still live \u2014 stop it first'); // the beacon owns the row until it stops (the menu's Stop row)
  else if (st === 200) { toast('deleted'); showList(); } // the ledger flipped — refresh now (the 15s beat would catch it anyway)
  else toast(jb?.error === 'no child' ? 'no such child' : 'delete failed');
}
async function resumeSession(id, pane) { // 7aq: POST /api/resume — the collector re-spawns the session's rpc agent on its recorded cwd; the beacon re-registers the SAME id, the row badge flips on the next 15s sidebar refresh
  try {
    ...
    if (r.status === 200) {
      ...
      if (sid === id) showSession(id, undefined, true); // stale pane truth: it IS live — reload the pane (force: the open pane's own re-select guard must not eat the 48 self-heal)
...
function renderCm() {
  ...
  if (cm.kind === 'session') {
    ...
    if (cm.live) cmRow(box, 'Stop', '', () => cmShutdown(cm.id)); // row-state: live children stop first — the delete 409's remedy (the Shutdown proxy works on any live beacon)
    cmRow(box, 'Delete', 'confirm', () => { cm.confirm = true; renderCm(); }); // row-state: ACTIVE — /api/delete-agent landed
    if (cm.confirm) { // the Shutdown confirm-row pattern, same treatment
      const cf = el('mconfirm', box);
      txt(el('mwarn', cf), 'Delete \u2014 removes the subagent record; the session file stays on disk.');
      const go = document.createElement('button'), no = document.createElement('button');
